...
<script type="text/javascript">
    ...
    function jump(id) {
        ...
        var teaId = form.getAttributeNode("teaId");
        var state = 0;
        if (buttonClicked === 1) {
            form.action = "teacher_modify.jsp?teaId=" + teaId;
            state = 1;
        } else {
            form.action = "/servlet/TeacherDeleteServlet?teaId=" + teaId;
            form.method = "get";
...
<div class="mdui-container">
    <table border="1" align="center" class="mdui-table mdui-table-hoverable mdui-typo">
        ...
        <%
            ...
            for (int i = 0; i < currentPageList.size(); i++) {
                TeacherBean teacher = (TeacherBean) currentPageList.get(i);
        %>
        <form id="form" onsubmit="return jump(<%=i%>)" action="">
            <tr>
                  
                <td><%=teacher.getTeaId() %><input value=<%=teacher.getTeaId()%> name="teaId" type="hidden"></td>
                  
                <td><%=teacher.getTeaName() %>
                </td>
                  
                <td><%=teacher.getTeaSex() %>
                </td>
    ...
    <div class="mdui-float-right mdui-icon-right mdui-m-a-2">
        <button class="mdui-btn mdui-color-theme-accent mdui-ripple" onclick="location.href='teacher_insert.jsp'">新增教师</button>
    </div>
</div>
